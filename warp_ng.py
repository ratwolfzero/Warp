...
def light_ray(x, f):
    """Simulate the positional dragging of light."""
...
for i, x0 in enumerate(x0_values):
    ...
    # Off-axis exotic energy density at rho = R
    rho = R
...
# Figure setup - 3 rows, single column
...
light_line1, = ax1.plot([], [], 'c-', lw=2, alpha=0.75, label="Light ray (dragged)")
...
theta_profile, = ax2.plot(x, theta_history[0, :], 'k-', lw=1.5, alpha=0.9)

# Bottom panel: Exotic Energy Density
ax3 = fig.add_subplot(gs[2])
energy_img = ax3.imshow(energy_history, aspect="auto", origin="lower",
                        extent=[X_MIN, X_MAX, 0, NUM_FRAMES],
                        cmap="plasma", vmin=energy_min, vmax=0)
ax3.set_title("Exotic Energy Density T₀₀ (negative, evaluated at ρ = R)")
...
energy_profile, = ax3.plot(x, energy_history[0, :], 'y-', lw=1.5, alpha=0.9)
...
def update(frame):
    ...
    ax1.set_title(f"Alcubierre Warp Bubble | x₀ = {x0:.1f} | v = {V_S}c")
